...
class AutoPoster:
    ...
    def __init__(self, config_path=CONFIG_PATH, cache_dir=CACHE_DIR, ipv6=False):
        ...
        if self.blacklist:
            ...
            log.info("Загружен черный список слов")
        ...
        if vk_token:  # Если в конфиге был указан токен, то используем его
            ...
            self.vk_session = VkApi(login=vk_login, password=vk_pass,
    ...
    def run(self):
        ...
        for domain in domains:
            ...
            disable_notification = self.config.getboolean(domain,
                                                          "disable_notification",
                                                          fallback=self.config.getboolean("global",
                                                                                          "disable_notification",
                                                                                          fallback=False))
            disable_web_page_preview = self.config.getboolean(domain,
                                                              "disable_web_page_preview",
                                                              fallback=self.config.getboolean("global",
                                                                                              "disable_web_page_preview",
                                                                                              fallback=True))
            send_stories = self.config.getboolean(domain, "send_stories",
                                                  fallback=self.config.getboolean("global", "send_stories",
                                                                                  fallback=False))
            ...
            send_reposts = self.config.get(domain, "send_reposts",
                                           fallback=self.config.get("global",
                                                                    "send_reposts", fallback=0))
            sign_posts = self.config.getboolean(domain, "sign_posts",
                                                fallback=self.config.getboolean("global",
                                                                                "sign_posts", fallback=True))
            what_to_parse = set(self.config.get(domain, "what_to_send",
                                fallback=self.config.get("global", "what_to_send",
                                                         fallback="all")).split(","))
            posts_count = self.config.getint(domain, "posts_count",
                                             fallback=self.config.get("global",
                                                                      "posts_count", fallback=11))
            last_story_id = self.config.getint(domain, "last_story_id", fallback=0)
            posts_id = self.get_posts_ids(domain)
    ...
    def get_posts_ids(self, domain: str) -> list:
        posts = self.config.get(domain, "posts_ids", fallback=0)
        if posts:
            return [int(post_id) for post_id in posts.split(':')]
        else:
            return []

    def set_posts_id(self, domain, posts_id):
        if len(posts_id) == 0:
            return
        elif len(posts_id) == 1:
